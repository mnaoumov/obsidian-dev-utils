...
- **Everything above is the CONSUMER side. The PROVIDER side: a plugin exposes a surface through `getPluginApis()` and nothing else.** Override it and the base does the rest — publishes each declaration against the gated wrapper component, revokes them when the feature surface unloads (never when the plugin unloads, so a consumer is not left holding a handle into a torn-down half), and derives the `plugin-loaded` broadcast's `apiVersions` from that same list. The asymmetry this replaces was measurable: five of the plugins built on this library have a public surface, and they had arrived at four different shapes — every one of them following the consumer-side rule above correctly, because the rule said nothing about the half they were on. Three shortcuts are wrong, and each is wrong in a way nothing type-checks:
  ...
  - **No instance `api` member either, not even as a second route.** The case for one is the consumer who cannot depend on a new enough copy of this library to have the registry — a real audience, which the published protocol now serves strictly better: it needs no library at all *and* it version-negotiates, where a getter hands back whatever happens to be installed and offers no way to ask which. A second route that skips both the contract validation and the broadcast makes the first route's guarantees optional for anyone who takes the shorter one.
- **A provider ships ONE hand-written, self-contained `api.d.ts` at the repo ROOT — both kinds of provider, including the core-widening kind below.** It is the only thing a stranger needs in order to type the surface, and the fixed name and location are what let them find it without reading the README first.
  - **Hand-written, and that is measured rather than preferred.** `tsc --emitDeclarationOnly` over a real API module emits imports of this library's own types (`PathOrFile`, `FolderNoteConfig`) and of sibling source modules (`EmptyFolderBehavior`). A stranger can resolve none of them, so a generated declaration is unusable for the one audience the file exists for. **Root, not `src/`**, for the same reason: it is a distributed artifact, not compiled source.
  - **Self-contained — it imports from `obsidian` and nothing else**, which every consumer already has. Do not reach for `@obsidian-typings/*` to pull in one type; inline it. **Nothing checks this, and that is worth knowing rather than assuming:** a plugin tsconfig sets `skipLibCheck: true`, which suppresses errors *inside* a declaration file entirely — a name `api.d.ts` cannot resolve is not reported at all (measured: TS2304 with `--skipLibCheck false`, silent with it on). So self-containment is read off the file's import list by a human, not proved by the gate.
  - **`src/<x>-api.ts` imports the interface from `api.d.ts` rather than re-declaring it**, so there is one declaration and nothing to drift. **The exact shape, measured on TypeScript 6.0.3 against a plugin tsconfig (`module`/`moduleResolution: node16`, `verbatimModuleSyntax`, `skipLibCheck: true`):**

    ```ts
    import type { MyPluginApi } from '../api.d.ts';
    ```

    - It compiles, and needs **neither** `allowImportingTsExtensions` nor `allowArbitraryExtensions` — both can be off. A `.d.ts` specifier is legal on its own terms, not on a flag a later tsconfig sweep could drop.
    - **The statement-level `import type` is mandatory.** The inline form `import { type MyPluginApi } from '../api.d.ts'` is **TS2846** — *a declaration file cannot be imported without `import type`* — and its "did you mean `../api.ts`" hint points at a file that does not exist. The shared ESLint config's `import-x/consistent-type-specifier-style` defaults to `prefer-top-level`, so lint asks for the one form the compiler requires; the two are not in tension here, which is what makes this safe to mandate at all.
    - **The extension is not optional.** A specifier with none — `'../api'` — is **TS2834** under `node16` resolution. `'../api.js'` also resolves — TypeScript finds `api.d.ts` beside the `api.js` that would be emitted — but it names a file the repo does not contain, so `'../api.d.ts'` is the specifier to write: it is the honest one, and it is the one that still turns up when someone greps for who reads the declaration.
    - **The build never sees it.** esbuild erases a type-only import at parse, so the emitted bundle carries no reference to `api.d.ts` and the specifier never reaches module resolution at build time (verified against the emitted bundle, not inferred).
    - **What the import DOES buy, `skipLibCheck` notwithstanding:** the implementation is still checked against the imported interface — a mismatched member is TS2322, naming the line in `api.d.ts` it violates. `skipLibCheck` suppresses errors inside a declaration file, never uses of one. So the single-declaration link is real; it is only the file's own internals that go unchecked.
  - **No npm publish.** The recorded reasoning against a shared protocol package applies here worse, not better: it rejected one because a publish treadmill and a second install were not worth two string constants, and a types package per provider is that same trade once per plugin that has a surface.
  - **The README links the file and stops re-typing the interface in prose**, which is the drift a single declaration exists to prevent. Which section, in what position, and with what contents is the shared plugin README skeleton's business, not this rule's.
- **The case the consumer side does not cover at all: a plugin that WIDENS a core Obsidian API.** Adding `.safe` / `.originalFn` and a `path` overload to `app.metadataCache.getBacklinksForFile`, or an overload to `app.fileManager.generateMarkdownLink`, is a different kind of surface: the consumer names *Obsidian*, and the plugin's presence only changes what that call can do. There is no handle to fetch, nothing to version-negotiate and nothing to revoke — the patch is installed or it is not. **So the registry is the wrong shape for it and must not be mandated**, which is a finding rather than an exemption: do not "fix" such a plugin by giving it a `getPluginApis()` override, because a consumer who fetched a handle would still make the call on `app`. What it owes a consumer is exactly one thing — the same root `api.d.ts`, giving the widened signature and naming **which plugin version added each member**, that being the only versioning a surface with no contract can offer.
- (cannot be forced by ESLint on the consumer side — an API-integration convention. The provider side is partly mechanizable and deliberately not mechanized: a custom rule could flag a `publishPluginApi` call outside this library, and a `public api` accessor or a public method on a `PluginBase` subclass, but it cannot tell a surface meant for other plugins from one meant for the plugin's own tests, which is the distinction the whole rule turns on)
