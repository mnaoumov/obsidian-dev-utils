 ...
 * outside this library. {@link errorToString} walks `errors` and prints the whole tree, so the console is
 * fine; everything that reads `message` is not. Obsidian's own plugin-load failure report, a `Notice`
 * built from an error, and a consumer's `rejects.toThrow(message)` all see an empty string and read it as
 * "nothing was thrown". Nesting compounds it: an aggregate holding one aggregate puts two empty layers
 * above the only sentence that says what went wrong.
 *
 ...
 * {@link errorToString} already prints it.
 *
