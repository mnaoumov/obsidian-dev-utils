...
require = Object.assign(require("import-sync"), require);
...
export async function buildObsidianPlugin({
  ...
  const buildOptions: BuildOptions = {
    ...
    format: "esm",
    logLevel: "info",
    outfile: "nothing-will-be-written-on-disk-so-this-does-not-matter",
    platform: "node",
    plugins: [
      convertToCjs(isProductionBuild, distDir, pluginName, obsidianConfigDir)
    ],
...
function convertToCjs(isProductionBuild: boolean, distDir: string, pluginName: string, obsidianConfigDir?: string): Plugin {
  return {
    name: "convert-to-cjs",
    setup(build) {
      build.onEnd(async (result) => {
        ...
        const esmText = result.outputFiles?.[0]?.text ?? "";
