...
import {
  applyContentChanges,
  applyFileChanges
} from '../../src/obsidian/file-change.ts';
import {
  convertLink,
  editBacklinks,
  editLinks,
...
} from '../../src/obsidian/metadata-cache.ts';
import {
  assertNonNullable,
  ensureGenericObject,
  ensureNonNullable
...
vi.mock('../../src/obsidian/metadata-cache.ts', async (importOriginal) => {
...
vi.mock('../../src/obsidian/file-change.ts', async (importOriginal) => {
  const original = await importOriginal();
  return {
    ...(original as object),
